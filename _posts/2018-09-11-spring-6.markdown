...
单例在Spring的同一个容器内只会被创建一次，后续获取bean会到单例缓存中尝试获取，如果获取不到，再尝试从singletonFactories中加载
```java
...
如果从缓存中获取了bean的原始状态，则需要对bean进行实例化。因为缓存中记录的只是bean的最原始状态，并不一定是最后需要得到的bean。比如工厂bean就需要得到factory-method方法返回的bean
```java
