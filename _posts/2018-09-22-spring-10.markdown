...
<font id="last-updated">最后更新于：2018-09-22</font>
...
1. 创建InvocationHandler接口实现类(代理执行类)
2. 实现invoke(Object proxy, Method method, Object[] args)方法
...
自定义标签解析，根据命名空间找到NamespaceHandler，即AopNamespaceHandler，执行其init方法
```java
public void init() {
...
注册了AspectJAutoProxyBeanDefinitionParser，BeanDefinitionParser接口从parse函数开始
```java
...
实现了BeanPostProcessor接口，所以当Spring预实例化bean后会调用其postProcessAfterInitialization方法
...
在实例化单例的时候，会调用resolveBeforeInstantiation方法，里面会调用applyBeanPostProcessorsBeforeInstantiation方法，获取到org.springframework.aop.aspectj.annotation.AnnotationAwareAspectJAutoProxyCreator(InstantiationAwareBeanPostProcessor)，调用它的postProcessBeforeInstantiation方法。在判断shouldSkip时，会去findCandidateAdvisors，那时候会调用buildAspectJAdvisors方法，所以在缓存中已经能取到aspectNames和cachedAdvisors了
```java
